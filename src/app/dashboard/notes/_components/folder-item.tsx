    ...
    <div className="flex h-full">
      ...
      <div className="w-3/4 pl-4 border-l overflow-y-auto">
        <Card>
          ...
          <CardContent>
            {(activeFolder ? activeFolderNotes : uncategorizedNotes).map(
              (note) => (
                <NoteItem
                  key={note.id}
                  note={note}
                  onUpdate={(title, content) =>
                    handleUpdateNote(note.id, title, content, note.folderId)
                  }
                  onDelete={() => handleDeleteNote(note.id)}
                />
              ),
            )}
            {(activeFolder ? activeFolderNotes : uncategorizedNotes).length ===
              0 && (
              <p className="text-muted-foreground">No notes in this folder.</p>
            )}
          </CardContent>
        </Card>
      </div>
