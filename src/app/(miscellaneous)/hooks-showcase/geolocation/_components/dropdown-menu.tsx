...
export default function DropdownMenu({ items, direction = 'vertical', className = '' }: DropdownMenuProps) {
        ...
        return (
            <li key={index} className="relative">
                <a
                    ...
                    className={`flex items-center px-4 py-2 text-subtitle hover:text-title hover:bg-section-lighter transition-colors ${hasSubMenu ? 'pr-8' : ''
                        }`}
                ...
                >
                    ...
                    {item.title}
                    {hasSubMenu && <ChevronRight className="absolute right-2 top-1/2 transform -translate-y-1/2" size={16} />}
                </a>
                {hasSubMenu && (
                    <AnimatePresence>
                        {activeSubMenu === index && (
    ...
    return (
        <div ref={menuRef} className={`relative ${className}`}>
            ...
            >
                {direction === 'horizontal' ? <MoreHorizontal /> : <MoreVertical />}
            </button>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
