...
export default function GeolocationUI() {
	...
	const dataPoints = [
		...
		},
		{
			icon: <Compass />,
			label: 'Accuracy',
			value: `${data.accuracy.toFixed(2)} meters`
		},
		{
			icon: <MapPin />,
			label: 'Altitude',
			value: data.altitude ? `${data.altitude.toFixed(2)} meters` : 'N/A'
		},
		{
			icon: <Compass />,
			label: 'Altitude Accuracy',
			value: data.altitudeAccuracy
				? `${data.altitudeAccuracy.toFixed(2)} meters`
				: 'N/A'
		},
		{
			icon: <Compass />,
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
			{dataPoints.map((point, index) => (
				<div
					key={index}
					className="flex items-center space-x-4 p-4 bg-section-lighter rounded-md border"
				>
