...
function ColorPickerModal({ color, onClose, onPickAgain }: { color: string; onClose: () => void; onPickAgain: () => void }) {
                ...
                <div className="grid gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="grid gap-1">
                            <div><span className="font-medium">HEX:</span> {hexColor}</div>
                            <div><span className="font-medium">RGB:</span> {rgbColor}</div>
                            <div><span className="font-medium">RGBA:</span> {rgbaColor}</div>
                            <div><span className="font-medium">Tailwind:</span> {tailwindClass}</div>
                        </div>
                    </div>
...
export default function ColorPicker() {
    ...
    const activatePicker = useCallback(() => {
        ...
        setTimeout(() => { // Use setTimeout to ensure the modal is closed before opening the eyedropper
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            bodyImage.onload = () => {
                ...
                ctx.drawImage(bodyCanvas, e.clientX, e.clientY, 1, 1, 0, 0, 1, 1)
