...
export function ActivityCard({
    ...
    return (
        <li className="shrink-0 basis-full lg:basis-[250px]">
            ...
            >
                <Icon name={icon} strokeWidth={2} size={20} className="text-white" />
    ...
    return (
        ...
        >
            <div className="flex flex-col gap-2 rounded-xl border bg-gray-100 p-6 dark:bg-gray-800">
                <div className={`h-5 w-5 rounded-full bg-gray-200 dark:bg-gray-700 ${animate ? 'animate-pulse' : ''}`} />
                <div className={`mt-2 h-6 w-full rounded bg-gray-200 dark:bg-gray-700 ${animate ? 'animate-pulse' : ''}`} />
                <div className={`mt-1 h-4 w-[50px] rounded bg-gray-200 dark:bg-gray-700 ${animate ? 'animate-pulse' : ''}`} />
            </div>
        </lia>
    )
}
