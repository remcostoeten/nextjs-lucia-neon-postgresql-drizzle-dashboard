...
export const reverseGeocode = cache(
	async (latitude: number, longitude: number): Promise<LocationInfo> => {
		try {
			...
			if (data.features.length === 0) {
				throw new Error(
					'No location data found for the given coordinates'
				)
			}
			...
			return {
				...
				city:
					properties.locality || properties.county || 'Unknown City',
