...
if (!IP_GEOLOCATION_API_KEY) {
	throw new Error(
		'IP_GEOLOCATION_API_KEY is not set in environment variables'
	)
}
...
async function getIPGeolocation(ip: string): Promise<any> {
	const response = await fetch(
		`https://api.ipgeolocation.io/ipgeo?apiKey=${IP_GEOLOCATION_API_KEY}&ip=${ip}`
	)
	if (!response.ok) {
		throw new Error(
			`Failed to fetch IP geolocation data: ${response.statusText}`
		)
	}
