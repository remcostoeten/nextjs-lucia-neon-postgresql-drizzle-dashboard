...
export const calculateDistance = cache(async (origin: string, destination: string): Promise<RouteInfo> => {
    try {
        ...
        const straightLineDistance = calculateStraightLineDistance(startCoords, endCoords)

        const routeResponse = await fetch(`https://api.openrouteservice.org/v2/directions/driving-car?api_key=${API_KEY}&start=${startCoords.join(',')}&end=${endCoords.join(',')}`)
